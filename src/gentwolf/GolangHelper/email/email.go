...
func SendSimple(addr string, auth smtp.Auth, m *Message) error {
...
func Send(config *Config, m *Message) error {
	...
	m.From = mail.Address{Name: strings.Split(config.From, "@")[0], Address: config.From}
	m.To = config.To
	return SendSimple(config.Smtp+":"+config.Port, auth, m)
}
...
func SendV2(config *ConfigV2, m *Message) error {
	auth := smtp.PlainAuth("", config.FromAddress, config.Password, config.Smtp)
	m.From = mail.Address{Name: config.FromName, Address: config.FromAddress}
	m.To = config.To
	return SendSimple(config.Smtp+":"+config.Port, auth, m)
}

func SendToV2(config *ConfigV2, toEmails []string, m *Message) error {
	auth := smtp.PlainAuth("", config.FromAddress, config.Password, config.Smtp)
	m.From = mail.Address{Name: config.FromName, Address: config.FromAddress}
	m.To = toEmails
	return SendSimple(config.Smtp+":"+config.Port, auth, m)
